...
static unsigned short rt5670_dsp_init_patch_code[][2] = { // patch for init time, which is auto generated from FM36600C_signoff0902.patch
...
#define RT5670_DSP_INIT_PATCH_NUM \
	(sizeof(rt5670_dsp_init_patch_code) / sizeof(rt5670_dsp_init_patch_code[0]))

static unsigned short rt5670_dsp_reset_patch_code[][2] = { // patch for switching mode, which is auto generated from fm36600c_signoff0902.patch
	{0xe1, 0x0010}, {0xe2, 0x0000}, {0xe0, 0x6ac9}, // set to dsp0
	{0xe1, 0x0064}, {0xe2, 0x0000}, {0xe0, 0x68c5}, // warm reset
	{0xe1, 0x3fa0}, {0xe2, 0x9183}, {0xe0, 0x3bcb},
	{0xe1, 0x3fb0}, {0xe2, 0x3f00}, {0xe0, 0x3bcb},
...
#define RT5670_DSP_RESET_PATCH_NUM \
	(sizeof(rt5670_dsp_reset_patch_code) / sizeof(rt5670_dsp_reset_patch_code[0]))
...
static unsigned short rt5670_dsp_16[][2] = {
	{0x22f2, 0x005c}, {0x2301, 0x0002},{0x22ec, 0x8000} // 20150312 higher sample rate
};
...
static unsigned short rt5670_dsp_ns[][2] = {
	...
	{0x2305, 0x0000}, {0x230c, 0x0800}, {0x22fb, 0x0000}
};
...
static unsigned short rt5670_dsp_aec[][2] = {
	...
	{0x2303, 0x0710}, {0x2304, 0x0332}, {0x230c, 0x0800}, {0x230d, 0x0080},
...
static unsigned short rt5670_dsp_ffp_ns[][2] = {
	...
	{0x2393, 0xfedc}, {0x2394, 0x8888}, {0x2395, 0x8888},{0x2396, 0x8832},
	{0x2397, 0x2224}, {0x2398, 0x4444},{0x22fb, 0x0000}
};
...
static unsigned short rt5670_dsp_48k_sto_ffp[][2] = {
	...
	{0x22c1, 0x1025}, {0x22c2, 0x1025}, {0x22f8, 0x8004}, {0x22ea, 0x0001}, //zzf
	{0x22c1, 0x1025}, {0x22c2, 0x1025}, {0x22f8, 0x8004}, {0x22ea, 0x0001},
	{0x230c, 0x0100}, {0x230d, 0x0100}, {0x2301, 0x0010}, {0x2303, 0x0200},
	//{0x2304, 0x8000}, {0x2305, 0x0000}, {0x2388, 0x6500}, {0x238b, 0x4000},
	{0x2304, 0xC000}, {0x2305, 0x0000}, {0x2388, 0x6500}, {0x238b, 0x4000}, //zzf
	{0x238c, 0x4000}, {0x23a8, 0x2800}, {0x23a9, 0x2000}, {0x23aa, 0x0200}, 
	{0x23ab, 0x7c00}, {0x2393, 0x8444}, {0x239c, 0x7000}, {0x22fb, 0x0000}
};
...
static unsigned short rt5670_dsp_aec_handsfree[][2] = {
	...
	{0x23a3, 0x4000}, {0x23ad, 0x2000}, {0x23ae, 0x2000}, {0x23af, 0x2000},
	{0x23b4, 0x2000}, {0x23b5, 0x2000}, {0x23b6, 0x2000}, {0x23bb, 0x6000},
	{0x2303, 0x0710}, {0x2304, 0x0332}, {0x230c, 0x0600}, {0x230d, 0x0080},
	{0x2310, 0x0010},{0x2393, 0xfedc}, {0x2394, 0x8888}, {0x2395, 0x8888},
	{0x2396, 0x8832},{0x2397, 0x2224}, {0x2398, 0x4444}, {0x22fb, 0x0000}
};
 ...
 * Returns 0 for success or negative error code.
 */
static int rt5670_dsp_do_patch(struct snd_soc_codec *codec, int mode)
{
	int ret, i;
	struct rt5670_priv *rt5670 = snd_soc_codec_get_drvdata(codec);
	...
	switch (mode) {
	case 0: /*Init*/
		for (i = 0; i < RT5670_DSP_INIT_PATCH_NUM; i++) {
			ret = snd_soc_write(codec, rt5670_dsp_init_patch_code[i][0],
				rt5670_dsp_init_patch_code[i][1]);
		if (ret < 0)
			goto patch_err;


			if (rt5670_dsp_init_patch_code[i][0] == 0xe0) {
				ret = rt5670_dsp_done(codec);
				if (ret < 0) {
					dev_err(codec->dev, "DSP is busy: %d\n", ret);
					goto patch_err;
		...
		break;
	case 1: /*Reset*/
		for (i = 0; i < RT5670_DSP_RESET_PATCH_NUM; i++) {
			ret = snd_soc_write(codec, rt5670_dsp_reset_patch_code[i][0],
				rt5670_dsp_reset_patch_code[i][1]);
			if (ret < 0)
				goto patch_err;


			if (rt5670_dsp_reset_patch_code[i][0] == 0xe0) {
				ret = rt5670_dsp_done(codec);
				if (ret < 0) {
					dev_err(codec->dev, "DSP is busy: %d\n", ret);
					goto patch_err;
...
static int rt5670_dsp_set_data_source(struct snd_soc_codec *codec, int src)
{
	int ret, i;
...
static int rt5670_dsp_set_mode(struct snd_soc_codec *codec, int mode)
{
	...
	if (rt5670_dsp_fw) {
		...
		printk("n=%d mode=%d\n", n, mode);
		if (mode <= n) {
			pos = rt5670_dsp_fw->data[(mode - 1) * 3 + 2] | 
				rt5670_dsp_fw->data[(mode - 1) * 3 + 1] << 8;
			
			tab_num = rt5670_dsp_fw->data[(mode - 1) * 3 + 3];
			printk("pos=%d tab_num=%d\n", pos, tab_num);

			i = 0;
			for (i = 0; i < tab_num; i++) {
				pr_debug("{0x%04x, 0x%04x}\n", 
					(rt5670_dsp_fw->data[pos + 1] << 8) | 
					rt5670_dsp_fw->data[pos + 2],
					(rt5670_dsp_fw->data[pos + 3] << 8) |
					rt5670_dsp_fw->data[pos + 4]);
				ret = rt5670_dsp_write(codec, 
					(rt5670_dsp_fw->data[pos + 1] << 8) | 
					rt5670_dsp_fw->data[pos + 2],
					(rt5670_dsp_fw->data[pos + 3] << 8) |
				...
				if (ret < 0)
					goto mode_err;
				pos += 5;
				if (rt5670_dsp_fw->size < pos)
					break;
			}
			goto set_dsp_mode_done;
		}
	}
...
static int rt5670_dsp_snd_effect(struct snd_soc_codec *codec)
{
	...
	snd_soc_update_bits(codec, RT5670_GEN_CTRL1, RT5670_RST_DSP,
		RT5670_RST_DSP);
	snd_soc_update_bits(codec, RT5670_GEN_CTRL1, RT5670_RST_DSP, 0);

	mdelay(10);

	if (!rt5670->dsp_inited) {
		ret = rt5670_dsp_do_patch(codec, 0);
		if (ret < 0)
			goto effect_err;
		rt5670->dsp_inited = true;
	}

	ret = rt5670_dsp_do_patch(codec, 1);
	if (ret < 0)
		goto effect_err;
	...
	ret = rt5670_dsp_rate(codec, 12288000, rate);
	if (ret < 0)
		goto effect_err;

	/*read MX-2d [3:2] to decide TDM source*/
	/*currently, support slot 0/1 and 2/3 only*/
	val = snd_soc_read(codec, RT5670_DSP_PATH1) & 0xc;
	if (val == 0x4) /*slot 2/3*/
		src = 1;

	ret = rt5670_dsp_set_data_source(codec, src);
	if (ret < 0)
		goto effect_err;
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (rt5670_dsp_fw) {
		n = rt5670_dsp_fw->data[0];
		if (rt5670->dsp_sw <= n) {
			pos = rt5670_dsp_fw->data[(rt5670->dsp_sw - 1) * 3 + 2] | 
				rt5670_dsp_fw->data[(rt5670->dsp_sw - 1) * 3 + 1] << 8;
			
			tab_num = rt5670_dsp_fw->data[(rt5670->dsp_sw - 1) * 3 + 3];
			printk("pos=%d tab_num=%d\n", pos, tab_num);
			...
			for (i = 0; i < tab_num; i++) {
				if (cnt + RT5670_DSP_REG_DISP_LEN >= PAGE_SIZE)
					break;
				val = rt5670_dsp_read(codec, 
					(rt5670_dsp_fw->data[pos + 1] << 8) | 
					rt5670_dsp_fw->data[pos + 2]);
				cnt += snprintf(buf + cnt, RT5670_DSP_REG_DISP_LEN,
					"%04x: %04x\n", 
					(rt5670_dsp_fw->data[pos + 1] << 8) |
					rt5670_dsp_fw->data[pos + 2], val);
				pos += 5;
				if (rt5670_dsp_fw->size < pos)
					break;
			}
...
show_dsp_mode_done:
	...
	for (i = 0; i < tab_num; i++) {
		if (cnt + RT5670_DSP_REG_DISP_LEN >= PAGE_SIZE)
			break;
		val = rt5670_dsp_read(codec, rt5670_dsp_rate_par[i]);
		cnt += snprintf(buf + cnt, RT5670_DSP_REG_DISP_LEN,
			"%04x: %04x\n",
			rt5670_dsp_rate_par[i], val);
	}
	if (cnt + RT5670_DSP_REG_DISP_LEN < PAGE_SIZE) {
		val = rt5670_dsp_read(codec, 0x3fb5);
		cnt += snprintf(buf + cnt, RT5670_DSP_REG_DISP_LEN,
			"%04x: %04x\n",
			0x3fb5, val);
	}
...
int rt5670_dsp_probe(struct snd_soc_codec *codec)
{
	...
	mdelay(10);
        ...
        if (rt5670_dsp_do_patch(codec, 0) < 0)
            dev_err(codec->dev, "Patch DSP rom code Fail !!!\n");

       //rt5670_dsp_rate(codec, 24576000, 16000);
	   //rt5670_dsp_rate(codec, 12288000, 16000);
       rt5670_dsp_rate(codec, 12288000, 48000); //zzf
        rt5670_dsp_conf(codec);
