...
#define RT5670_ALC_DRC_REG_LEN ARRAY_SIZE(hpmic_list)
static int rt5670_hpmic_init(struct snd_soc_codec *codec)
{
	int i;

	for (i = 0; i < RT5670_INIT_REG_LEN; i++)
		snd_soc_write(codec, hpmic_list[i].reg, hpmic_list[i].val);
	return 0;
}
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	case RT5670_ASRC_EN://enable ASRC
		...
		snd_soc_write(codec, RT5670_ADDA_CLK1, 0x0010);
		break;
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		snd_soc_write(codec, RT5670_ALC_DRC_CTRL2, 0x061f); // preboost 18db
		snd_soc_write(codec, RT5670_ALC_CTRL_1, 0x222c);
             #endif
		snd_soc_update_bits(codec, RT5670_PDM_OUT_CTRL,
			RT5670_M_PDM1_R, 0);
	...
	struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i <= RT5670_VENDOR_ID2; i++) {
		...
		if (rt5670_readable_register(codec, i)) {
...
static DEVICE_ATTR(codec_reg_adb, 0666, rt5670_codec_adb_show, rt5670_codec_adb_store);
...
static int rt5670_probe(struct snd_soc_codec *codec)
{
	...
	if (rt5670->v_id >= 7) {
		...
		snd_soc_dapm_new_controls(&codec->dapm, rt5670_old_dapm_widgets,
